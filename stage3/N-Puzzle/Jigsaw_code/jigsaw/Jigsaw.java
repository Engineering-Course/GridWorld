...
public class Jigsaw {
	...
	private void estimateValue(JigsawNode jNode) {
		int s = (int) (3 * estimateValue1(jNode) + 1 * estimateValue2(jNode) +
				       7 * estimateValue3(jNode) + 10 * estimateValue4(jNode));
	...
	private int estimateValue3(JigsawNode jNode) {
		...
		for (int i = 1; i <= dimension * dimension; i++) {
			if (jNode.getNodesState()[i] != i && jNode.getNodesState()[i] != 0) {
				int currentRow = (jNode.getNodesState()[i] - 1) / dimension;
				int currentCol = (jNode.getNodesState()[i] - 1) % dimension;
				int endRow = (i - 1) / dimension;
				int endCol = (i - 1) % dimension;
				s += Math.abs((currentRow - endRow) + Math.abs(currentCol - endCol));
			}
		}
		return s;
	}
	//  记录几何距离
	private int estimateValue4(JigsawNode jNode) {
		int s = 0;
		int dimension = JigsawNode.getDimension();
		for (int i = 1; i <= dimension * dimension; i++) {
			if (jNode.getNodesState()[i] != i && jNode.getNodesState()[i] != 0) {
				...
				int endRow = (i - 1) / dimension;
				int endCol = (i - 1) % dimension;
				s += Math.sqrt((currentRow - endRow) * (currentRow - endRow) +
						      (currentCol - endCol) * (currentCol - endCol));
			}
		}
		return s;
	}
